...
class dbManager
{
		...
		if(!ISSET($_COOKIE["useralmacen"])){
			setcookie("page", $page);	
  			 header('Location: LogIn.php');
				...
				if($page=="autap.php"){
						if($_COOKIE["nivel"]!=md5('1A')&&$_COOKIE["nivel"]!=md5('1B')){
								setcookie("page", "");	
  			              header('Location: LogIn.php');						
   ...
   public function getProovedores() {
		...
		public function getProovedoresBusq($nkey) {
			$nkey=strtoupper($nkey);
			$uss=array("","","","");		
			
			require('settings.php');
	      $sql="SELECT MD5(idproveedor) as idproveedor ,nombre, descripcion, direccion FROM inv_proveedores WHERE borrado='0' AND nombre LIKE '%".$nkey."%';"; 
	      $dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
		...
		public function agregaArticulo($n, $des,$ip1,$p1,$ip2,$p2,$ip3,$p3,$code){
			require('settings.php');
	      $sql="INSERT INTO inv_articulos (nombre,descripcion,id_proveedor1,precio1,id_proveedor2,precio2,id_proveedor3,precio3,code) VALUES('".$n."','".$des."','".$ip1."','".$p1."','".$ip2."','".$p2."','".$ip3."','".$p3."','".$code."');"; 
	      $dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
		}
		...
		public function getProductosB($key){
			$key=strtoupper($key);
			$uss=array("","","","","","","","","");		
			require('settings.php');
	      $sql="select MD5(idarticulo) as idarticulo , nombre, descripcion,(SELECT nombre from inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor1) as prov1, precio1,(SELECT nombre from inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor2) as prov2, precio2,(SELECT nombre from inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor3) as prov3, precio3 from inv_articulos WHERE borrado='0' AND upper(inv_articulos.nombre) LIKE '%".$key."%' ORDER BY nombre"; 
			//echo($sql);
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
		...
		public function getProductosByCode($key){
			$key=strtoupper($key);
			$uss=array("","","","","","","","","");		
			require('settings.php');
	      $sql="select MD5(idarticulo) as idarticulo , nombre, descripcion,(SELECT nombre from inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor1) as prov1, precio1,(SELECT nombre from inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor2) as prov2, precio2,(SELECT nombre from inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor3) as prov3, precio3 from inv_articulos WHERE borrado='0' AND upper(inv_articulos.code) LIKE '%".$key."%' ORDER BY nombre"; 
			//echo($sql);
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
		...
		public function getProducto($idarticulo){
			...
			$uss=array("","","","","","","","","","");		
			require('settings.php');
	      $sql="select MD5(idarticulo) as idarticulo , nombre, descripcion,id_proveedor1 as prov1, precio1,id_proveedor2 as prov2, precio2,id_proveedor3 as prov3, precio3, idarticulo as idart from inv_articulos WHERE MD5(idarticulo)='".$idarticulo."'"; 
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
		...
		public function getAlmacen(){
			require('settings.php');
			$uss=array("","","","","","");
			$msquery="SELECT inv_articulos.nombre, inv_almacen.disponible,(SELECT inv_proveedores.nombre FROM inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor1) as Proveedor1,(SELECT inv_proveedores.nombre FROM inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor2) as Proveedor2,(SELECT inv_proveedores.nombre FROM inv_proveedores WHERE inv_proveedores.idproveedor=inv_articulos.id_proveedor3) as Proveedor3, inv_articulos.code FROM inv_almacen INNER JOIN inv_articulos ON inv_articulos.idarticulo=inv_almacen.idarticulo ORDER BY inv_almacen.disponible";
		        $dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
		$result = pg_query($msquery) or die('La consulta fallo: ' . pg_last_error());		
		while($row=pg_fetch_assoc($result)){
					
					//echo('Hola');
				
	             $uss[0]=$uss[0].",".$row['nombre'];
	             $uss[1]=$uss[1].",".$row['disponible']; 
	            
	             $uss[2]=$uss[2].",".$row['proveedor1'];
	             $uss[3]=$uss[3].",".$row['proveedor2']; 
	             $uss[4]=$uss[4].",".$row['proveedor3'];
		     $uss[5]=$uss[5].",".$row['code'];
				}
		...
		public function getRequicisiones(){
			require("settings.php");
			$uss= array("","","","","","","");
			$sql="SELECT idrq, inv_articulos.nombre,cantidad, costo,fecha,(SELECT p.nombre FROM inv_proveedores p WHERE p.idproveedor=r.id_proveedor) AS proveedor, inv_usuarios.nombre || ' ' || inv_usuarios.paterno || ' ' || inv_usuarios.materno as aprueba FROM inv_req_compras r INNER JOIN inv_articulos ON inv_articulos.idarticulo=r.id_articulo INNER JOIN inv_usuarios ON inv_usuarios.idusuario=r.id_usuario_libera WHERE r.realizada=0";
			//echo($sql);
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
	             ...
	             $uss[2]=$uss[2].",".$row['cantidad'];
	             $uss[3]=$uss[3].",".$row['costo']; 
	             $uss[4]=$uss[4].",".$row['fecha'];
	             $uss[5]=$uss[5].",".$row['proveedor'];
		     $uss[6]=$uss[6].",".$row['aprueba']; 
				}
		...
		public function getRequicisionesN(){
			require("settings.php");
			$uss= array("","","","","","","");
			$sql="SELECT idrq, inv_articulos.nombre,cantidad, costo,fecha,(SELECT p.nombre FROM inv_proveedores p WHERE p.idproveedor=r.id_proveedor) AS proveedor, inv_usuarios.nombre || ' ' || inv_usuarios.paterno || ' ' || inv_usuarios.materno as aprueba FROM inv_req_compras r INNER JOIN inv_articulos ON inv_articulos.idarticulo=r.id_articulo LEFT JOIN inv_usuarios ON inv_usuarios.idusuario=r.id_usuario_libera WHERE r.liberada=0 AND r.id_usuario_libera is null";
			//echo($sql);
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
		...
		public function getRequicisionesIds($ids){
			require("settings.php");
			$uss= array("","","","","","","");
			$sql="SELECT idrq, inv_articulos.nombre,cantidad, costo,fecha,(SELECT p.nombre FROM inv_proveedores p WHERE p.idproveedor=r.id_proveedor) AS proveedor, inv_usuarios.nombre || ' ' || inv_usuarios.paterno || ' ' || inv_usuarios.materno as aprueba FROM inv_req_compras r INNER JOIN inv_articulos ON inv_articulos.idarticulo=r.id_articulo INNER JOIN inv_usuarios ON inv_usuarios.idusuario=r.id_usuario_libera WHERE r.realizada=0 AND idrq IN (".$ids.")";
			//echo($sql);
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
		...
		public function realizaRequicisiones($ids){
			require("settings.php");
			$sql="UPDATE inv_req_compras SET realizada=1 WHERE idrq IN (".$ids.")";
			//echo($sql);
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
		}
		public function getEntradas(){
			require("settings.php");
			$uss= array("","","","");
			$sql="SELECT inv_entradas.fecha,inv_articulos.nombre,inv_entradas.cantidad,inv_usuarios.nombre || ' ' || inv_usuarios.paterno || ' ' || inv_usuarios.materno  AS usuariorecibe  FROM inv_entradas INNER JOIN inv_articulos ON inv_articulos.idarticulo=inv_entradas.idarticulo INNER JOIN inv_usuarios ON inv_usuarios.idusuario =inv_entradas.id_usuario ORDER BY inv_entradas.fecha";
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
	             ...
	             $uss[0]=$uss[0].",".$row['fecha'];
	             $uss[1]=$uss[1].",".$row['nombre']; 
	            
	             $uss[2]=$uss[2].",".$row['cantidad'];
	             $uss[3]=$uss[3].",".$row['usuariorecibe']; 
		...
		public function getSalidas(){
			require("settings.php");
			$uss= array("","","","","","","","");
			$sql="SELECT inv_salidas.fecha,inv_articulos.nombre,inv_salidas.cantidad,inv_almacen.disponible,ur.nombre || ' ' || ur.paterno || ' ' || ur.materno AS usuariorecibe, ue.nombre || ' ' || ue.paterno || ' ' || ue.materno AS usuarioentrega, CASE WHEN ur.idusuario is null THEN 'n' ELSE 's' END AS entregada,CASE WHEN inv_salidas.cantidad > inv_almacen.disponible THEN 's' ELSE 'n' END AS req_ap  FROM inv_salidas INNER JOIN inv_articulos ON inv_articulos.idarticulo=inv_salidas.id_articulo INNER JOIN inv_usuarios ue ON ue.idusuario=inv_salidas.id_usuario_entrega LEFT JOIN inv_usuarios ur ON ur.idusuario=inv_salidas.id_usuario_recibe LEFT JOIN inv_almacen ON inv_almacen.idarticulo=inv_salidas.id_articulo ORDER BY inv_salidas.fecha";
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
	             ...
	             $uss[2]=$uss[2].",".$row['cantidad'];
	             $uss[3]=$uss[3].",".$row['disponible'];
		      $uss[4]=$uss[4].",".$row['usuariorecibe'];
	             $uss[5]=$uss[5].",".$row['usuarioentrega'];
		     $uss[6]=$uss[6].",".$row['entregada'];
		     $uss[7]=$uss[7].",".$row['req_ap'];
		...
		public function getReqComprasR(){
			require("settings.php");
			$uss= array("","","","","","","","");
			$sql="SELECT  inv_req_compras.fecha as fechaaprobacion, inv_req_compras.fecha_envio as fechapeticion, inv_articulos.nombre,inv_req_compras.cantidad,inv_req_compras.costo,ul.nombre || ' ' || ul.paterno || ' ' || ul.materno AS usuariolibera,CASE WHEN inv_req_compras.liberada=1 THEN 's' ELSE 'n' END as liberada,utp.nombre || ' '|| utp.paterno || ' ' || utp.materno AS usuariocrea FROM inv_req_compras INNER JOIN inv_articulos ON inv_articulos.idarticulo=inv_req_compras.id_articulo LEFT JOIN inv_usuarios ul ON ul.idusuario=inv_req_compras.id_usuario_libera INNER JOIN inv_usuarios utp ON utp.idusuario=inv_req_compras.id_usuario_toma_peticion";
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
	             ...
	             $uss[0]=$uss[0].",".$row['fechaaprobacion'];
	             $uss[1]=$uss[1].",".$row['fechapeticion']; 
	            
	             $uss[2]=$uss[2].",".$row['nombre'];
	             $uss[3]=$uss[3].",".$row['cantidad'];
		      $uss[4]=$uss[4].",".$row['costo'];
	             $uss[5]=$uss[5].",".$row['usuariolibera'];
		     $uss[6]=$uss[6].",".$row['liberada'];
		     $uss[7]=$uss[7].",".$row['usuariocrea'];
		...
		public function generaCodigo(){
			require("settings.php");
			$cod="";
			$sql="SELECT MAX(idarticulo) as max FROM inv_articulos";
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
		...
		public function validaCodigo($cod){
			...
			$count=0;
			$resp=true;
			$sql="SELECT COUNT(*) as max FROM inv_articulos WHERE code='".$cod."'";
			$dbconn = pg_pconnect($strCnx) or die('No se ha podido conectar: ' . pg_last_error());
			$result = pg_query($sql) or die('La consulta fallo: ' . pg_last_error());
			while($row=pg_fetch_assoc($result)){
					
				$count=intval($row["max"]);
